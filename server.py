...
ensure_stock_seeded(sorted(ALLOWED_PRODUCT_IDS))
...
async def stock_one(product_id: str):
    return {"product_id": product_id, "quantity": get_stock(product_id)}


@app.websocket("/ws/updates")
async def ws_updates(websocket: WebSocket):
    """Live feed for events that matter to *every* connected shopper right
    now - stock changes chief among them. Multiple browser tabs/customers can
    be looking at the same product; without this, one person adding the last
    unit to their bag is invisible to everyone else until they refresh.

    Implemented over the same Redis instance used for caching/session state
    (a `PUBLISH`/`SUBSCRIBE` channel) rather than introducing a separate
    message broker - at this traffic scale a dedicated Kafka cluster would be
    pure operational overhead for the same result.
    """
    ...
    listener = threading.Thread(
        target=subscribe_events_blocking,
        args=(_on_message, stop_requested.is_set),
        daemon=True,
    )
    listener.start()
