...
def classify_workflow(user_text: str) -> WorkflowSpec:
    workflow, _score = classify_workflow_with_score(user_text)
    return workflow


def classify_workflow_with_score(user_text: str) -> Tuple[WorkflowSpec, int]:
    text = (user_text or "").lower()
    if not text.strip():
        return WORKFLOW_INDEX["master_entry"], 0
    ranked = sorted(WORKFLOWS, key=lambda workflow: _score_workflow(text, workflow), reverse=True)
    best = ranked[0]
    score = _score_workflow(text, best)
    if score <= 1:
        return WORKFLOW_INDEX["master_entry"], score
    return best, score


def classify_conversation(recent_user_messages: List[str]) -> Tuple[WorkflowSpec, int]:
    """Classify using recent user turns instead of only the single latest
    message. The latest message is weighted most heavily (repeated in the
    scored text) but short follow-ups like "yes", "the blue one", or "how
    much?" get to keep the context of what was said 1-2 turns earlier instead
    of being scored on their own and landing back on `master_entry`.
    """
    messages = [m for m in recent_user_messages if m and m.strip()]
    if not messages:
        return WORKFLOW_INDEX["master_entry"], 0
    latest = messages[-1]
    context = messages[-4:-1]
    combined = " ".join(context) + " " + (latest + " ") * 3
    return classify_workflow_with_score(combined)
