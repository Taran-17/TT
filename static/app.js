...
// ==========================================================================
// LIVE UPDATES (stock changes, etc) - so a second shopper looking at the
// same product sees a change without refreshing, even though their own
// chat turn didn't cause it.
// ==========================================================================
function connectLiveUpdates() {
    try {
        ...
        socket.onmessage = (event) => {
            ...
            if (payload.event_type === 'stock_update') {
                const { product_id, remaining_stock } = payload.payload || {};
                if (product_id) {
                    handleStockUpdate(product_id, remaining_stock);
                }
            }
        };
        socket.onclose = () => {
            // Best-effort reconnect; a dropped socket shouldn't break the chat.
...
function handleStockUpdate(productId, remainingStock) {
    if (remainingStock === null || remainingStock === undefined) return;
    document.querySelectorAll(`[data-product-id="${productId}"] .stock-badge`).forEach(badge => {
        badge.textContent = remainingStock > 0 ? `${remainingStock} left` : 'Sold out';
    });
    if (remainingStock <= 3) {
        showToast(remainingStock > 0
            ? `Only ${remainingStock} left of that item - someone else may be looking at it too.`
...
function addMessageToChat(role, content, actions = []) {
    ...
    if (role === 'assistant') {
        // 1. Interactive Visual Option Cards - driven strictly by the
        // `present_options` action the agent actually returned. (This used to
        // also guess options from substrings like "occasion"/"fabric"/"fit"
        // in the assistant's own text, which meant almost every reply
        // triggered a card grid regardless of whether the agent asked a
        // question at all - that guesswork is gone.)
        ...
        // 2. Visual Product Recommendation Cards - only when the agent
        // actually returned a recommendation action, not just whenever the
        // word "suit" or "recommend" appeared in a sentence.
        ...
        // 3. Visual Fabric Swatch Gallery Cards - only when the agent issued
        // a customize_fabric action (previously any message merely
        // containing the word "fabric" rendered this whole gallery).
        if (actions.some(a => a.type === 'customize_fabric')) {
            const fabrics = [
        ...
        // 5. Cart / Checkout Widget Card - only on an actual cart action.
        if (actions.some(a => ['add_to_bag', 'open_cart'].includes(a.type))) {
            widgetHTML += `
